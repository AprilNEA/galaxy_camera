...
//        LOG(INFO) << w_str << " set failed!";
        std::cout << w_str << " set failed!"<< std::endl;
    }
}
...
void GX_STDC OnFrameCallbackFun(GX_FRAME_CALLBACK_PARAM *pFrame) {
    ...
    if (pFrame->status == GX_FRAME_STATUS_SUCCESS) {
        ...
        // 疑似使用 Cuda 加速，待研究
        // galaxy_camera->full_gpu.upload(galaxy_camera->full);
        // cv::cuda::resize(galaxy_camera->full_gpu,galaxy_camera->resize_gpu,cv::Size(640,640));
        // cv::cuda::cvtColor(galaxy_camera->resize_gpu,galaxy_camera->resize_gpu,cv::COLOR_RGB2BGR);
        ...
        if (camera->is_energy) {
            ...
            cv::resize(camera->p_energy, camera->p_img, cv::Size(640, 384),
                       cv::INTER_NEAREST);
            ...
            camera->pimg_lock.lock();
            // cv::resize(galaxy_camera->full,galaxy_camera->p_img,cv::Size(640,640),cv::INTER_NEAREST);
            // cv::cvtColor(galaxy_camera->p_img,galaxy_camera->p_img,cv::COLOR_RGB2BGR);
            // galaxy_camera->resize_gpu.download(galaxy_camera->p_img);
...
//                    std::chrono::duration_cast<std::chrono::milliseconds>(end - galaxy_camera->fps_time_point).count();
//            LOG(INFO) << "average hkcamera delay(ms):"
//                      << galaxy_camera->frame_get_time / galaxy_camera->frame_cnt
...
//void getRGBImage(DHCamera *galaxy_camera) {
//    while (true) {
//        if (!galaxy_camera->thread_running) {
//            return;
//        }
//        GX_STATUS status;
//        // 进行相机数据采集
//        status = GXDQBuf(galaxy_camera->g_hDevice, &galaxy_camera->g_frameBuffer, 1000);
//
//        auto start = std::chrono::steady_clock::now();
//
//        // galaxy_camera->g_frameBuffer.
//        ProcessData(galaxy_camera->g_frameBuffer->pImgBuf,
//                    galaxy_camera->g_pRaw8Buffer,
//                    galaxy_camera->g_pRGBframeData,
//                    galaxy_camera->g_frameBuffer->nWidth,
//                    galaxy_camera->g_frameBuffer->nHeight,
//                    galaxy_camera->g_frameBuffer->nPixelFormat,
//                    galaxy_camera->g_nColorFilter);
//
//        if (galaxy_camera->is_energy) {
//            // LOG(INFO) << galaxy_camera->g_frameBuffer->nWidth << " " <<
//            // galaxy_camera->g_frameBuffer->nHeight << " " << galaxy_camera->nPayLoadSize;
//            memcpy(galaxy_camera->p_energy.data, galaxy_camera->g_pRGBframeData, 3 * (galaxy_camera->nPayLoadSize));
//
//            galaxy_camera->pimg_lock.lock();
//
//            cv::resize(galaxy_camera->p_energy, galaxy_camera->p_img, cv::Size(640, 384), cv::INTER_NEAREST);
//            cv::cvtColor(galaxy_camera->p_img, galaxy_camera->p_img, cv::COLOR_RGB2BGR);
//
//            galaxy_camera->pimg_lock.unlock();
//        } else {
//            galaxy_camera->pimg_lock.lock();
//
//            memcpy(galaxy_camera->p_img.data, galaxy_camera->g_pRGBframeData,3 * (galaxy_camera->nPayLoadSize));
//            cv::cvtColor(galaxy_camera->p_img, galaxy_camera->p_img, cv::COLOR_RGB2BGR);
//
//            galaxy_camera->pimg_lock.unlock();
//        }
//        GXQBuf(galaxy_camera->g_hDevice, galaxy_camera->g_frameBuffer);
//        auto end = std::chrono::steady_clock::now();
//        galaxy_camera->frame_cnt++;
//        galaxy_camera->frame_get_time +=
//                std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
//                        .count();
//        if (galaxy_camera->frame_cnt == 500) {
//            std::cout << "average galaxy_camera delay(ms):"
//                      << galaxy_camera->frame_get_time / galaxy_camera->frame_cnt << std::endl;
//            galaxy_camera->frame_get_time = galaxy_camera->frame_cnt = 0;
//        }
//    }
//}
...
DHCamera::DHCamera(std::string sn)
        : sn(std::move(sn)),
          ...
          init_success(false),
          is_energy(false) {
    p_img = cv::Mat(384, 640, CV_8UC3);
    p_energy = cv::Mat(384, 640, CV_8UC3);
};
