...
typedef enum GX_DEVICE_CLASS_LIST
{
    ...
    GX_DEVICE_CLASS_SMART               = 4,           ///< Smart galaxy_camera device
...
typedef enum GX_FEATURE_ID
{
    ...
    GX_FLOAT_GAIN                     = 5011 | GX_FEATURE_FLOAT | GX_FEATURE_LEVEL_REMOTE_DEV,  ///< The value is an float value that sets the selected gain control in units specific to the galaxy_camera.
...
{
    char              *pszContent;                       ///< Standard C string that is decided by openMode. It could be an IP address, a galaxy_camera serial number, and so on
...
\brief      Initialize the device library for some resource application operations. This interface must be called
            before using the GxIAPI to interact with the galaxy_camera, and the GXCloseLib must be called to release all
...
//----------------------------------------------------------------------------------
/**
\brief      Inquire the current galaxy_camera whether support a special feature. Usually the galaxy_camera does not support a
            feature means that:
            1) By inquiring the galaxy_camera register, the current galaxy_camera really does not support this feature.
            2) There is no description of this feature in the current galaxy_camera description file.
\param      [in]hDevice             The handle of the device.
\param      [in]featureID           The feature code ID.
...
//----------------------------------------------------------------------------------
/**
\brief      At present, the Mercury Gigabit galaxy_camera provides the device offline notification event mechanism, the
...
//----------------------------------------------------------------------------------
/**
\brief      Export the current parameter of the galaxy_camera to the configuration file.(ANSI)
\param      [in]hDevice             The handle of the device.
...
//----------------------------------------------------------------------------------
/**
\brief      Import the configuration file for the galaxy_camera.(ANSI)
\param      [in]hDevice             The handle of the device.
...
// ---------------------------------------------------------------------------
/**
\brief      Configure the static IP address of the galaxy_camera.
